    ...
    <div class="max-w-7xl mx-auto px-4 lg:px-8 py-4 flex items-center justify-between">
        ...
        <div class="flex items-center gap-4 w-1/4 justify-end min-w-0">
            ...
            @auth
                ...
                <!-- Notifications -->
                <div class="relative">
                    <button id="notification-dropdown-button" type="button" class="relative text-slate-500 hover:text-blue-600 transition p-2 rounded-xl hover:bg-slate-50 cursor-pointer">
                        <svg class="w-6 h-6" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"></path>
                        </svg>
                        @if($unreadNotificationCount > 0)
                            <span class="absolute -top-0.5 -right-0.5 min-w-5 h-5 px-1 inline-flex items-center justify-center bg-red-500 text-white text-[10px] font-bold rounded-full ring-2 ring-white">
                                {{ $unreadNotificationCount > 99 ? '99+' : $unreadNotificationCount }}
                            </span>
                        @endif
                    </button>

                    <div id="notification-dropdown-menu" class="absolute right-0 top-full mt-2.5 w-[22rem] max-w-[calc(100vw-1.5rem)] bg-white border border-slate-100 rounded-3xl shadow-2xl shadow-slate-200/40 hidden opacity-0 translate-y-1 transition-all duration-200 z-50 overflow-hidden">
                        <div class="p-4 border-b border-slate-100 bg-gradient-to-br from-slate-50 to-white">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="text-xs text-slate-500 mt-1">{{ $unreadNotificationCount > 0 ? $unreadNotificationCount.' belum dibaca' : 'Belum ada notifikasi baru' }}</p>
                                </div>
                                <div class="flex items-center gap-2">
                                    ...
                                    @if($unreadNotificationCount > 0)
                                        <form method="POST" action="{{ route('notifications.read-all') }}">
                                            @csrf
                                            <button type="submit" class="text-xs font-semibold text-slate-500 hover:text-slate-700 cursor-pointer">
                                                Tandai semua dibaca
                                            </button>
                                        </form>
                                    @endif
                        ...
                        <div class="max-h-96 overflow-y-auto divide-y divide-slate-100">
                            @forelse($notifications as $notification)
                                ...
                                <div class="p-3.5 {{ $isUnread ? 'bg-blue-50/60' : 'bg-white' }}">
                                    <div class="flex gap-3">
                                        <div class="shrink-0 w-10 h-10 rounded-2xl border {{ $toneClasses[0] }} flex items-center justify-center">
                                        ...
                                        <div class="min-w-0 flex-1">
                                            <div class="flex items-start justify-between gap-2">
                                                <div class="min-w-0">
                                                    <a href="{{ $url }}" class="block text-sm font-semibold text-slate-900 hover:text-blue-700 transition truncate">
                                                        {{ $title }}
                                                    </a>
                            ...
                            @empty
                                <div class="p-6 text-center">
                                    <div class="w-12 h-12 mx-auto rounded-2xl bg-slate-50 text-slate-400 flex items-center justify-center">
                ...
                <div class="flex items-center gap-3 border-l border-slate-100 pl-4 relative">
                    ...
                    <!-- Dropdown Menu -->
                    <div id="user-dropdown-menu" class="absolute right-0 top-full mt-2.5 w-56 bg-white border border-slate-100 rounded-2xl shadow-xl shadow-slate-200/50 hidden opacity-0 translate-y-1 transition-all duration-200 z-[100] p-1.5 space-y-0.5">
                        <a href="{{ route('customer.profile') }}" class="group/item flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-slate-700 hover:text-blue-600 hover:bg-slate-50 transition-all duration-200">
                        ...
                        <a href="{{ route('customer.settings') }}" class="group/item flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 text-slate-700 hover:text-blue-600 hover:bg-slate-50">
                            <svg class="w-[18px] h-[18px] text-slate-400 group-hover/item:text-blue-500 transition-colors duration-200" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
...
<<<<<<< HEAD
                        <a href="{{ route('pembayaran.index') }}" class="group/item flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium {{ Route::currentRouteName() === 'pembayaran.index' ? 'text-blue-600 bg-blue-50/50' : 'text-slate-600' }} hover:text-blue-600 hover:bg-blue-50/50 transition-all duration-200">
                            <svg class="w-[18px] h-[18px] {{ Route::currentRouteName() === 'pembayaran.index' ? 'text-blue-500' : 'text-slate-400' }} group-hover/item:text-blue-500 transition-colors duration-200" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
=======
                        <a href="{{ route('customer.payments') }}" class="group/item flex items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-semibold transition-all duration-200 text-slate-700 hover:text-blue-600 hover:bg-slate-50">
                            <svg class="w-[18px] h-[18px] text-slate-400 group-hover/item:text-blue-500 transition-colors duration-200" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
>>>>>>> 4c94e9f25750fb5f60022eda23ef5d3c47557ac6
                        ...
                        <form method="POST" action="{{ route('logout') }}">
                            ...
                            <button type="submit" class="group/item flex w-full items-center gap-3 px-3.5 py-2.5 rounded-xl text-sm font-medium text-red-600 hover:text-red-700 hover:bg-red-50/60 transition-all duration-200 cursor-pointer">
                                <svg class="w-[18px] h-[18px] text-red-500 group-hover/item:text-red-600 transition-colors duration-200" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.25 9.75L19.5 12m0 0l-2.25 2.25M19.5 12H9m10.5-9v4.5m0 9V21M3 6.75h1.5M3 17.25h1.5" />
