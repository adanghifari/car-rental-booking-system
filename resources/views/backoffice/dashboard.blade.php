<x-backoffice.layout title="Backoffice Dashboard" :admin="$admin" active="dashboard">
    ...
    <section class="stats-grid">
        ...
        <div class="card stat-card">
            <div class="stat-top">
                <div class="stat-icon" style="background: rgba(32, 38, 54, 0.08); color: #202636;">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                        <path
                            d="M14 16H9m10 0h2m-7 0h1m-9 0h1m0 0a2 2 0 1 0 4 0m-4 0a2 2 0 1 1 4 0m8 0a2 2 0 1 0 4 0m-4 0a2 2 0 1 1 4 0M3 12l2-5h13l3 5" />
                    </svg>
                </div>
            </div>
        ...
        <div class="card stat-card">
            ...
            <div class="stat-value">Rp{{ number_format($stats['monthly_revenue'] / 1000, 0) }}k</div>
        </div>
        ...
        <div class="card">
            ...
            <div class="chart-shell">
                <svg class="chart-svg" viewBox="0 0 680 280" preserveAspectRatio="none" aria-hidden="true">
                    ...
                    @for ($i = 0; $i
                    < 5; $i++) <line x1="28" y1="{{ 36 + ($i * 46) }}" x2="652" y2="{{ 36 + ($i * 46) }}"
                        stroke="#e5ebf4" stroke-dasharray="6 8" />
                    @endfor

                    @php
                    $rentalPoints = collect($chartRentals)->values()->map(function ($point, $index) use
                    ($maxRentalValue) {
                    $x = 44 + ($index * 118);
                    ...
                    <polyline points="{{ $polyline }}" fill="none" stroke="#3f5ed7" stroke-width="4"
                        stroke-linecap="round" stroke-linejoin="round"></polyline>

                    @foreach ($rentalPoints as $point)
                    <circle cx="{{ $point['x'] }}" cy="{{ $point['y'] }}" r="4" fill="#3f5ed7"></circle>
                    <text x="{{ $point['x'] }}" y="258" fill="#7b869b" font-size="12"
        ...
        <div class="card status-card">
            ...
            <div class="donut-wrap">
                <svg width="220" height="220" viewBox="0 0 180 180" aria-hidden="true">
                    ...
                    <text x="90" y="88" text-anchor="middle" font-size="28" font-weight="700"
                        fill="#202636">{{ $stats['total_cars'] }}</text>
    ...
    <section class="bottom-grid">
        <div class="card">
            ...
            <div class="chart-shell" style="min-height: 260px;">
                <svg class="chart-svg" viewBox="0 0 420 250" preserveAspectRatio="none" aria-hidden="true">
                    ...
                    $x = 34 + ($index * 62);
                    $y = 190 - $barHeight;
                    $isActive = $index === collect($chartRevenue)->search(fn ($item) => $item['value'] ===
                    collect($chartRevenue)->max('value'));
                    @endphp
                    <rect x="{{ $x }}" y="{{ $y }}" width="38" height="{{ $barHeight }}" rx="10"
                        fill="{{ $isActive ? '#3f5ed7' : '#e8eef8' }}"></rect>
                    <text x="{{ $x + 19 }}" y="222" fill="#7b869b" font-size="12"
                        text-anchor="middle">{{ $point['label'] }}</text>
                    @endforeach
                </svg>
            </div>
        ...
        <div class="card">
            <div class="section-head">
                ...
                <a href="#" style="color: var(--blue); font-size: 13px; font-weight: 700; text-decoration: none;">Lihat
                    Semua</a>
            </div>
            ...
            <table class="table">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            <span
                                class="pill {{ $activity['status']['tone'] }}">{{ $activity['status']['label'] }}</span>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
            ...
            <div class="vehicle-stage">
                <svg viewBox="0 0 560 320" aria-hidden="true">
                    ...
                    <text x="454" y="276" fill="#111827" font-size="18" font-weight="700"
                        text-anchor="end">{{ $stats['total_cars'] }} Units</text>
                </svg>
            </div>
        </div>
    </section>
